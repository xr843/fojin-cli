...
fn fixture() -> Connection {
    ...
    conn.execute("INSERT INTO norm_map(from_char,to_char) VALUES ('應','应')", []).unwrap();
    ...
    conn.execute("INSERT INTO norm_map(from_char,to_char) VALUES ('無','无')", []).unwrap();
    conn.execute(
...
fn compute_applies_normalization() {
    ...
    assert!(out.contains("apratiṣṭhita"), "traditional query should match folded zh_norm");
}
